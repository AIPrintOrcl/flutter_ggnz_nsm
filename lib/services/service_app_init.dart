...
class ServiceAppInit extends GetxService {
  ...
  void _checkWalletAddress() async {
    ...
    // 상황에 맞추어 페이지 이동
    // 로그인 여부와 지갑 비번 생성 여부에 따라 페이지 이동.
      ...
      Future.delayed(
          const Duration(seconds: 5),
              () => Get.off(() => const PasswordPage(),
                  ...
                  transition: Transition.fadeIn,
                  duration: const Duration(milliseconds: 500)));
    } else if (getx.isWalletExist) {
      // 사용자 전환 안내 및 비밀번호 설정 페이지로 이동
      // 안내 메시지 1
      var result_1 = await Get.to(() => SignWalletPage(signReason: 'change_user_to_v2_1'.tr),
          arguments: {"reason": "change_user_to_v2_1"});

      if (result_1) {
        // 안내 메시지 2
        var result_2 = await Get.to(() => SignWalletPage(signReason: 'change_user_to_v2_2'.tr),
  ...
  void _checkFirebaseFireStore() async {
    ...
    await Future.delayed(Duration(seconds: 5)); /* 로딩 시간 */
